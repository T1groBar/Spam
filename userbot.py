...
async def answer(_, message):
    ...
    if message.edit_date is None or (
        ...
        logger.debug(f"|{message.chat.title}|СКРИПТ ПРОИГНОРИРОВАЛ (edit date: {message.edit_date})")
        return

    if last_media_group == message.media_group_id and message.media_group_id is not None:
        logger.debug(
            f"|{message.chat.title}|СКРИПТ ПРОИГНОРИРОВАЛ (media group: last - {last_media_group}, message - {message.media_group_id}).")
        return

    logger.info(f"|{message.chat.title}|:ПОСТ КАНАЛА (текст: {message_log})")
    ...
    if message_to_answer is None:
        logger.error(f"|{message.chat.title}|:БАН/ВЫКЛ. КОММ")
        return
